...
class CrewRegistrationForm(FlaskForm):
    ...
    available_port_city = StringField('Available Port / City for Joining', validators=[Optional(), Length(max=128)])
...
class CrewProfileDocumentForm(FlaskForm):
    """Form for crew members to upload multiple documents per category"""
    # Identity Documents - Multiple file uploads
    passport = MultipleFileField('Passport Copy', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    government_id = MultipleFileField('Government ID (Aadhaar / PAN / SSN)', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    photo = MultipleFileField('Photo (Passport Size)', validators=[Optional(), FileAllowed(['jpg', 'jpeg', 'png'], 'Image files only!')])
    
    # Medical Documents - Multiple file uploads
    medical_certificate = MultipleFileField('Medical Certificate', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    yellow_fever = MultipleFileField('Yellow Fever Certificate', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    
    # Professional Documents - Multiple file uploads
    cdc = MultipleFileField('CDC (Seaman Book)', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    coc_cop = MultipleFileField('COC/COP Certificate', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    stcw_certificates = MultipleFileField('STCW Certificates', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    gmdss_dce = MultipleFileField('GMDSS/DCE Certificate', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
    
    # Other Documents - Multiple file uploads
    resume = MultipleFileField('Resume/CV', validators=[Optional(), FileAllowed(['pdf', 'doc', 'docx'], 'PDF and Word documents only!')])
    sea_agreement = MultipleFileField('SEA Agreement', validators=[Optional(), FileAllowed(['pdf', 'jpg', 'jpeg', 'png'], 'PDF and image files only!')])
